...
//     char numbers[] = "Chen Junxin : 23336040";
...
extern "C" void c_time_interrupt_handler()
{
    ...
    // 滚动显示
    int pos = times % (2 * length);  
    for (int i = 0; i < length; ++i)  
    {  
        if (pos < length)  
        {  
            // 前半段：字符串向左滚动消失  
            result[i] = (i < pos) ? ' ' : numbers[i - pos];  
        }  
        else  
        {  
            // 后半段：字符串从右侧逐渐出现  
            int actualPos = pos - length;  
            result[i] = (i < actualPos) ? numbers[length - actualPos + i] : ' ';  
        }  
    }  
    result[length] = '\0';  
